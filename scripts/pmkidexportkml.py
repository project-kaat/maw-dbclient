...
cur.execute("SELECT bssid, channel, last_time_seen, wps, roaming, cracked FROM network WHERE bssid IN (SELECT bssid FROM location) AND bssid IN (SELECT bssid FROM pmkid)") #grab networks that have at least one location
